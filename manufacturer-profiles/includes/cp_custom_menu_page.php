<?php 
    ...
    echo '<div style="margin-left: 10px;">';
        for ($y = 0; $y < $page_numbers; $y+=$pnumber) {
            ...
            if ($pp == $current_page){
                if($_POST && isset($_POST['psearch'])){
                    echo '<a href="'.get_site_url().'/wp-admin/admin.php?page=cpcustompage&psearch='.$_POST['psearch'].'&pnumber='.$pp.$pregion .'"><span>'.strval(intval($y/$pnumber + 1)).'</span></a>&nbsp;';
                }
                elseif (isset($_GET['psearch'])){
                    echo '<a href="'.get_site_url().'/wp-admin/admin.php?page=cpcustompage&psearch='.$_GET['psearch'].'&pnumber='.$pp.$pregion .'"><span>'.strval(intval($y/$pnumber + 1)).'</span></a>&nbsp;';
                }
                else{
                    echo '<a href="'.get_site_url().'/wp-admin/admin.php?page=cpcustompage&pnumber='.$pp.$pregion .'"><span>'.strval(intval($y/$pnumber + 1)).'</span></a>&nbsp;';
                }
            }else {
                    if($_POST && isset($_POST['psearch'])){
                    echo '<a style="color:black;" href="'.get_site_url().'/wp-admin/admin.php?page=cpcustompage&psearch='.$_POST['psearch'].'&pnumber='.$pp.$pregion .'"><span>'.strval(intval($y/$pnumber + 1)).'</span></a>&nbsp;';
                    }
                    elseif (isset($_GET['psearch'])){
                        echo '<a style="color:black;" href="'.get_site_url().'/wp-admin/admin.php?page=cpcustompage&psearch='.$_GET['psearch'].'&pnumber='.$pp.$pregion .'"><span>'.strval(intval($y/$pnumber + 1)).'</span></a>&nbsp;';
                    }
                    else {
                        echo '<a style="color:black;" href="'.get_site_url().'/wp-admin/admin.php?page=cpcustompage&pnumber='.$pp.$pregion .'"><span>'.strval(intval($y/$pnumber + 1)).'</span></a>&nbsp;';
                    } 
                }
        }
    echo '</div><br>';
    ...
    if ($cp_result->num_rows > 0) {
        echo '<table><tr><td>Manufacturer Name</td><td><a href="'.get_site_url().'/wp-admin/admin.php?page=cpcustompage&loc=1">Location</a></td><td></td><td></td></tr>';
            ...
            while($cp_row = $cp_result->fetch_assoc()) {
                    ...
                    if ( in_array( 'mfp_owner', (array) $user->roles ) ) {
                        ...
                        if ($comapnyOwnerId == $currentUserId) {
                            ...
                            echo '<td><a href="'.get_site_url().'/wp-admin/admin.php?page=cpcustomsubpage&company_id='.$cp_row["company_id"].'">Edit</a></td>';
                            $cp_row["name"] = str_replace(',','',$cp_row["name"]);
                            $cp_row["name"] = str_replace(' ','-',$cp_row["name"]);
                            echo '<td><a href="'.get_site_url().'/brands/'.preg_replace("/[^A-Za-z0-9-]/", '', $cp_row["name"]).'" target=”_blank”>View Page</a></td>';
                            echo "</tr>";
                        }
                    }else{
        ...
        echo '<div style="margin-left: 10px;">';
            for ($x = 0; $x < $page_numbers; $x+=$pnumber) {
                ...
                if ($p == $current_page){
                    echo '<a href="'.get_site_url().'/wp-admin/admin.php?page=cpcustompage&pnumber='.$p.$pregion.'"><span>'.strval(intval($x/$pnumber + 1)).'</span></a>&nbsp;';
                } 
                else {
                    echo '<a style="color:black;" href="'.get_site_url().'/wp-admin/admin.php?page=cpcustompage&pnumber='.$p.$pregion.'"><span>'.strval(intval($x/$pnumber + 1)).'</span></a>&nbsp;';
                }     
            }
        echo '</div>';
    }
